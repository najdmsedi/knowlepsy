...
export const PulseIndicator = () => {
  ...
  const expo = useImage(require("./assets/favicon.png"));
  const heart = useImage(require("./assets/favicon.png"));

  const interval = 1250;

  const scale = useComputedValue(() => {
    return ((clock1.current % interval) / interval) * 50;
  }, [clock1]);
  ...
  const scale2 = useComputedValue(() => {
    return (((clock1.current + 400) % interval) / interval) * 60;
  }, [clock1]);
  ...
  return (
    <Canvas style={{ height: 300, width: 300 }}>
      <Circle cx={150} cy={150} r={5} opacity={1} color="#FF6060"></Circle>
